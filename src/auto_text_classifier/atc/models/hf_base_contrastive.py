...
from atc.utils.data_utils import load_df, load_df_paired
...
class HFBase(BaseModel):
    ...
    def get_data_generator_paired(self, data):
        # 现在data 是一个list，里面包含(t, t_neg, label)
        input_ids, attention_masks, input_ids_neg, attention_masks_neg, label_list = self.process_one_data_paired(data)
        data_input = TensorDataset(input_ids, attention_masks, input_ids_neg, attention_masks_neg, label_list)
        data_sampler = RandomSampler(data_input)
        data_dataloader = DataLoader(
            data_input, sampler=data_sampler, batch_size=self.batch_size)
        return data_dataloader

    def process_one_data_paired(self, data):
        # Tokenize all of the sentences and map the tokens to thier word IDs.
        # 这个函数是专门处理成对输入的[(text, text_neg, label)]
        # 输出应该是5个东西，input_ids, attention_masks,
        # input_ids_neg, attention_masks_neg, label_list
        ...
        # For every sentence...
        for text, text_neg, label in data:
            text_list.append(text)
            text_neg_list.append(text_neg)
            label_list.append(label)

        input_ids, attention_masks = self.process_text_list(text_list)
        input_ids_neg, attention_masks_neg = self.process_text_list(text_neg_list)
        label_list = torch.tensor(label_list)
        return input_ids, attention_masks, input_ids_neg, attention_masks_neg, label_list
    ...
    def process_data(self, train_path, dev_path, test_path):
        train_data = self.load_data_contrastive(train_path)
        dev_data = self.load_data_contrastive(dev_path)

        # test_data不变.
        test_data = self.load_data(test_path)

        # train_generator 和 dev_generator 要改成pair 输入.
        # update: 不pair输入，而是两列，第一列是正例，第二列是负例.
        # 现在train_data 是一个list，里面包含(t, t_neg, label)

        train_generator = self.get_data_generator_paired(train_data)
        dev_generator = self.get_data_generator_paired(dev_data)
    ...
    def train_model(self, train_generator, dev_generator, test_generator):
        ...
        for epoch_i in range(0, epochs):
            ...
            for step, batch in enumerate(train_generator):
                ...
                b_input_ids_neg = batch[2].to(self.device)
                b_input_mask_neg = batch[3].to(self.device)
                ...
                self.margin = 1 # control margin.
                self.gamma = 0.5

                if self.token_type_ids_disable:
                    outputs = self.model(b_input_ids,
                                        attention_mask=b_input_mask)
                    outputs_neg = self.model(b_input_ids_neg,
                                        attention_mask=b_input_mask_neg)
                ...
                loss_2 = torch.sum(torch.pow(torch.clamp(self.margin - euclidean_distance, min=0.0), 2))

                loss = loss_1 * (1 - self.gamma) + loss_2 * self.gamma
                ...
                print('loss_1 item:', loss_1.item())
                print('loss_2 item:', loss_2.item())
