...
# import sys
# sys.path.append('/share/small_project')
...
from atc.configs.emb_config import cn_wikipedia_emb_config
# Torch数据集
...
class TorchBase(BaseModel):
    def __init__(self, O_CONFIG):
        ...
        cn_wikipedia_emb_config['emb_path'] = '/share/small_project/auto_text_classifier/atc/data/word_vector/wikipedia/wiki-news-300d-1M.pkl'
        O_CONFIG = {
            "b_print": True,
            "run_logger": None,
            "stop_token": ["", " ", " ", "\t", "\r", "\n"],
            ...
            "use_cuda": True,
            "update_embed": True,
            "save_dir": "/workspace/projects_2020/auto_text_classifier/tmp_multitask_1210/bilstm"
        }
    ...
    def get_torch_data(self, train_path, dev_path, test_path):
        # 训练集  进行文本预处理 text -> token -> id vector
        ...
        # 测试集  进行文本预处理 text -> token -> id vector
    ...
    def train(self, train_path, dev_path, test_path): 
        ...
        # 因为类别不平衡，所以配置权重  类别不平衡比例 0/1 <0.5 或 0/1 > 2
